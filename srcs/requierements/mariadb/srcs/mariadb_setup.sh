...
mysql -u root -e "ALTER USER root@localhost IDENTIFIED WITH 'pass';"
...
mysql -u root -p${ROOTPASSWORD} -e "GRANT USAGE ON $DB_DATABASE.* TO '$DB_USERNAME'@'%' IDENTIFIED BY '$DB_PASSWORD' WITH GRANT OPTION"
mysql -u root -p${ROOTPASSWORD} -e "GRANT ALL PRIVILEGES ON $DB_DATABASE.* TO '$DB_USERNAME'@'%' IDENTIFIED BY '$DB_PASSWORD' WITH GRANT OPTION;"

mysql -u root --password=${ROOTPASSWORD} $DB_DATABASE < export.sql

mysql -u root -p${ROOTPASSWORD} -e "INSERT INTO $DB_DATABASE.wp_users (ID, user_login, user_pass, user_nicename, user_email, user_url, user_registered, user_activation_key, user_status, display_name) VALUES (1,	'$WP_ROOT_USERNAME', MD5('$WP_ROOT_PASSWORD'), '$WP_ROOT_USERNAME', 'amonteli@student.42lyon.fr', 'https://amonteli.42.fr', '2022-04-12 09:53:18', '',	0, '$WP_ROOT_USERNAME');"
mysql -u root -p${ROOTPASSWORD} -e "INSERT INTO $DB_DATABASE.wp_users (ID, user_login, user_pass, user_nicename, user_email, user_url, user_registered, user_activation_key, user_status, display_name) VALUES (2,	'wpuser', MD5('$WP_USER_PASSWORD'), '$WP_ROOT_USERNAME', 'user@student.42lyon.fr', 'https://amonteli.42.fr', '2022-04-12 09:53:18', '',	0, 'wpuser');"
